...
NEURON {
	...
	POINTER ko3, nao3, kid, kod, naid, naod, clid, clod, caid, caod
...
KINETIC state {
	...
	shell[0]  = rad*Difk*(geom(diam)/2)*surf(diam)*(ko3-k[1]-kbuf)
	shell[1]  = narad*Difna*(geom(diam)/2)*surf(diam)*(nao3-na[1])
