...
NEURON {
	...
	POINTER koint, ko1, ko3, naoint, nao1, nao3, kid, kod, naid, naod, clid, clod, caid, caod
...
KINETIC state {
	...
	shell[1]  = rad*Difk*(geom(diam)/2)*surf(diam)*(ko1-k[1]-kbuf)
	shell[2]  = rad*Difk*(geom(diam)/2)*surf(diam)*(ko3-k[1]-kbuf)
	shell[3]  = narad*Difna*(geom(diam)/2)*surf(diam)*(naoint-na[1])
	shell[4]  = narad*Difna*(geom(diam)/2)*surf(diam)*(nao1-na[1])
	shell[5]  = narad*Difna*(geom(diam)/2)*surf(diam)*(nao3-na[1])
