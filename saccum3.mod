...
NEURON {
	...
	POINTER koint, ko2, ko4, naoint, nao2, nao4, kid, kod, naid, naod, clid, clod, caid, caod
...
KINETIC state {
	...
	shell[0]  = rad*Difk*(geom(diam)/2)*surf(diam)*(koint-k[1]-kbuf)
	shell[1]  = rad*Difk*(geom(diam)/2)*surf(diam)*(ko2-k[1]-kbuf)
	shell[2]  = rad*Difk*(geom(diam)/2)*surf(diam)*(ko4-k[1]-kbuf)
	shell[3]  = narad*Difna*(geom(diam)/2)*surf(diam)*(naoint-na[1])
	shell[4]  = narad*Difna*(geom(diam)/2)*surf(diam)*(nao2-na[1])
	shell[5]  = narad*Difna*(geom(diam)/2)*surf(diam)*(nao4-na[1])
	...
	~  a[0] << (aflux[0])
	~  na[1] << (naflux[1]+dif[0]+shell[3]+shell[4]+shell[5]+nalat[1])
	~  k[1]  << (kflux[1]+dif[1]+shell[0]+shell[1]+shell[2]+klat[1])
