...
NEURON {
	...
	POINTER ko1, ko3, nao1, nao3, kis, kos, nais, naos, clis, clos, cais, caos
...
KINETIC state {
	...
	shell[0]  = rad*Difk*(geom(diam)/2)*surf(diam)*(ko1-k[1]-kbuf)
	shell[1]  = rad*Difk*(geom(diam)/2)*surf(diam)*(ko3-k[1]-kbuf)
	shell[2]  = narad*Difna*(geom(diam)/2)*surf(diam)*(nao1-na[1])
